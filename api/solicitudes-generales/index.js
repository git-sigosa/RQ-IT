...
async function handleCreate(context, req, respond) {
  ...
  try {
    ...
    const result = await request.query(
      ...
      '(Titulo, Solicitante, Area, Ambito, Descripcion, Justificacion, Prioridad, Estado, ProyectoId, PayloadJson) ' +
      'OUTPUT INSERTED.Id ' +
      'VALUES (@Titulo, @Solicitante, @Area, @Ambito, @Descripcion, @Justificacion, @Prioridad, @Estado, @ProyectoId, @PayloadJson)'
    );
