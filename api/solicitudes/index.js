...
/* GET /api/solicitudes/{id} · detalle completo (incluye PayloadJson) */
async function handleGetOne(context, req, respond) {
  ...
  try {
    ...
    const result = await r.query('SELECT * FROM dbo.Solicitudes WHERE Id = @Id');
    const row = result.recordset && result.recordset[0];
    if (!row) return respond(404, { ok: false, error: 'Solicitud no encontrada.' });
    return respond(200, { ok: true, item: row });
  } catch (err) {
    context.log.error('Error al obtener detalle: ' + err.message);
